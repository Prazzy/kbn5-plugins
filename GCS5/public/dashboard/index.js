  ...
  return {
    ...
    controller: function ($scope, $rootScope, $route, $routeParams, $location, Private, getAppState, savedDashboards) {
      ...
      function getDashList(params) {
        let filters = queryFilter.getGlobalFilters();
        let filter_operator = '';
        let filter_menu_group = '';
        ...
        let appTitle = $scope.chrome.getAppTitle();
        let appTitle_lst = appTitle.split('_');
        let app = appTitle_lst[0];
        let multiple_operators = 0;
      ...
      //     var esRes = getDashList(params);
      //     kbnUrl.change('/dashboard/'.concat(esRes[0].id));
